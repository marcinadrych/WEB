...
export default function Dashboard() {
  ...
  useEffect(() => { /* logika skanera bez zmian */ if (isScannerOpen) { if (!scannerRef.current) { scannerRef.current = new Html5QrcodeScanner(qrcodeRegionId, { fps: 10, qrbox: { width: 250, height: 250 } }, false); } scannerRef.current.render(onScanSuccess, onScanFailure); } else { if (scannerRef.current && scannerRef.current.getState() === 2) { scannerRef.current.clear().catch(e => {}); } } return () => { if (scannerRef.current && scannerRef.current.getState() === 2) { scannerRef.current.clear().catch(e => {}); } }; }, [isScannerOpen]);
  function onScanSuccess(decodedText) { setSearchTerm(decodedText); setIsScannerOpen(false); }
  function onScanFailure(error) {}
  async function getProducts() { setLoading(true); const { data } = await supabase.from('produkty').select('*').order('kategoria').order('podkategoria').order('nazwa'); setProducts(data || []); setLoading(false); }

  const filteredProducts = useMemo(() => { if (!searchTerm.trim()) return []; const keywords = searchTerm.toLowerCase().split(' ').filter(Boolean); return products.filter(p => { if (String(p.id) === searchTerm) return true; const text = [p.nazwa, p.kategoria, p.podkategoria || ''].join(' ').toLowerCase(); return keywords.every(k => text.includes(k)); }); }, [products, searchTerm]);
  const groupedProducts = useMemo(() => { return products.reduce((acc, p) => { const cat = p.kategoria; const sub = p.podkategoria || 'Bez podkategorii'; if (!acc[cat]) acc[cat] = {}; if (!acc[cat][sub]) acc[cat][sub] = []; acc[cat][sub].push(p); return acc; }, {}); }, [products]);
